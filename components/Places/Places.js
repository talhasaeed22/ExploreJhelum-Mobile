    ...
    const getPosts = async () => {
        ...
        firestore().collection('Places').get().then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                const { name, image, number, location, map, link, feedback, desc } = doc.data();
                postItem.push({
                    id: doc.id,
                    name: name,
                    image: image,
        ...
        <>
            ...
            <ScrollView>
                <View style={{ padding: 20 }}>
                    <Text style={{ color: "rgb(191, 28, 28)", fontSize: 30, fontWeight: 'bold', textDecorationLine: "underline", }}>All Places</Text>
                </View>
                ...
                {loading ? 
                 ...
                 : posts.map((item) => {
                    return <View style={{marginVertical:20}} key={item.id}>
                       <PlaceBox navigation={navigation} name={item.name} path={item.image[0]} element={item} />
                    </View>
                })
                }
