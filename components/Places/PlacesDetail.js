import { View, Text, Image, Dimensions, ScrollView, TouchableOpacity, Linking, TextInput, Alert } from 'react-native'
...
const RestaurantDetail = ({ navigation, route }) => {
  ...
  const handleComments = () => {
    firestore().collection('Places').doc(element.id).update({
  ...
  const getComments = async () => {
    ...
    firestore().collection('Places').get().then((snapshot) => {
